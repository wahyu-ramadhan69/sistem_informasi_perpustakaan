...
class User extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->data['petugas'] =  $this->db->query("SELECT * FROM tbl_login WHERE level = 'Petugas' ");
		$this->data['title_web'] = 'Data User ';
		$this->load->view('user/user.php', $this->data);
	}
	public function anggota()
	{
		$this->data['idbo'] = $this->session->userdata('ses_id');
		$this->data['user'] = $this->M_Admin->get_table('tbl_login');
		$this->data['anggota'] =  $this->db->query("SELECT * FROM tbl_login WHERE level = 'Anggota' ");
		$this->data['title_web'] = 'Data User ';
		$this->load->view('user/anggota.php', $this->data);
	}
	...
	public function tambahang()
	{
		...
		$this->data['nomer'] = $id;
		$this->load->view('user/tambah_ang', $this->data);
	}
	...
	public function add()
	{
		...
		if ($dd->num_rows() > 0) {
			$this->session->set_flashdata('error', 'Username / Email Anda Sudah Terpakai');
	...
	public function addang()
	{
		...
		if ($dd->num_rows() > 0) {
			$this->session->set_flashdata('error', 'Username / Email Anda Sudah Terpakai');
			redirect(base_url('user/tambahang'));
		} else {
			// setting konfigurasi upload
			$nmfile = "user_" . time();
			$config['upload_path'] = './assets_style/image/';
	...
	public function editang()
	{
		...
		$this->data['title_web'] = 'Edit User ';
		$this->load->view('user/editang', $this->data);
	}
		...
		if (!$this->upload->do_upload('gambar')) {
			if ($this->input->post('pass') !== '') {
				...
				if ($this->session->userdata('level') == 'Petugas') {
					...
					redirect(base_url('user/editang/' . $id_login));
				}
			} else {
				$data = array(
					'nama' => $nama,
