...
class Transaksi extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('level') == 'Anggota') {
			$this->data['pinjam'] = $this->db->query(
				"SELECT DISTINCT `pinjam_id`, `anggota_id`, `buku_id`,
			...
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `anggota_id`, `buku_id`,
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM tbl_pinjam WHERE status = 'Dipinjam' ORDER BY id_pinjam DESC");
		}

		$this->load->view('pinjam/pinjam', $this->data);
	}
	...
	function cetak_pinjam()
	{
		...
		$this->load->view('laporan/transaksi',  $this->data);
	}
	...
	public function kembali()
	{
		...
		$this->load->view('kembali/kembali', $this->data);
	}
	...
	public function pinjam()
	{
		...
		$this->load->view('pinjam/tambah', $this->data);
	}
	...
	public function peminjaman()
	{
		...
		$tgl2 = date('Y-m-d', strtotime('+' . $this->input->post('lama') . ' days', strtotime($tgl)));
		...
		$this->db->insert_batch('tbl_pinjam', $data);
		$this->session->set_flashdata('pesan', '<div id="notifikasi"><div class="alert alert-success"><p> Tambah Pinjam Buku Sukses !</p></div></div>');
		redirect(base_url('transaksi/'));
	}
	...
	public function denda()
	{
		...
		$this->load->view('denda/denda', $this->data);
	}
	...
	function loaduser()
	{
		$sql = $this->db->query("SELECT * FROM tbl_login")->result();
		echo json_encode($sql);
	}

	function loadbuku()
	{
		$sql = $this->db->query("SELECT * FROM tbl_buku")->result();
		echo json_encode($sql);
	}
